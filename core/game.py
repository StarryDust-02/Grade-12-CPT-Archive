...
class Game(object):
    ...
    def check_bound(self, group, bound): 
        for sprite in range(len(group)):
            if group[sprite].positon.x < bound[0] or group[sprite].position.y < bound[1] or group[sprite].position.x > bound[2] or group[sprite].position.y > bound[3]:
                return group.pop(sprite)
